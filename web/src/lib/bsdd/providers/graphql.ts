...
export class GraphqlBsddProvider implements BsddProvider {
  ...
  async searchClasses(term: string, dicts: string[], limit = 20): Promise<BsddClass[]> {
    ...
    const fields: string[] = [];
    const variables: Record<string, unknown> = { searchText: trimmed };
    dicts.forEach((uri, i) => {
      const alias = `d${i}`;
      const varName = `u${i}`;
    ...
    const query = `query Search($searchText: String!, ${dicts
      .map((_, i) => `$u${i}: String!`)
      .join(", ")}) {
      ${fields.join("\n")}
    }`;
  ...
  async getClass(uri: string): Promise<BsddClassDetail | null> {
    ...
    const query = `query ($dictionaryUri: String!, $uri: String!) {
      dictionary(uri: $dictionaryUri) {
        uri
        class(uri: $uri, includeChildren: false) {
          name
          referenceCode
          uri
        }
    ...
    const data = await gqlFetch<{ dictionary?: { class?: any } }>({
      query,
      variables: { dictionaryUri: dictUri, uri },
    });
